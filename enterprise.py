# NICEGOLD Enterprise Supergrowth v2.0
# Risk sizing, trend-based reward/risk, OMS enhancements, full logging
import pandas as pd
import numpy as np
import os
...
def calc_indicators(df):
    ...
    plus_di = 100 * plus_dm.rolling(adx_period).sum() / atr_adx
    minus_di = 100 * minus_dm.rolling(adx_period).sum() / atr_adx
...
def smart_entry_signal(df):
    ...
    long_cond = trend_long & (df['rsi'] > 53) & (vol_high | adx_good)
    short_cond = trend_short & (df['rsi'] < 47) & (vol_high | adx_good)
...
def run_backtest():
    ...
    print(df_trades[['entry_time', 'type', 'entry', 'exit', 'pnl', 'capital', 'mode', 'reason_entry', 'reason_exit']].tail(12))
