...
class TestIndicators(unittest.TestCase):
    @unittest.skipUnless(
        pandas_available and numpy_available, "requires pandas and numpy"
    )
...
class TestNicegoldExtra(unittest.TestCase):
    ...
    def test_detect_macd_divergence_values(self):
        df = pd.DataFrame(
            {"close": [1, 2, 0.5, 2, 3, 4], "macd_hist": [0, -1, 1, -2, 2, -3]}
        )
    ...
    def test_should_force_entry(self):
        ...
        self.assertFalse(
            nicegold.should_force_entry(row, last_entry, current_time_short)
        )
    ...
    def test_should_force_entry_branches(self):
        ...
        now = datetime.now()
        self.assertFalse(
            nicegold.should_force_entry(row, now - timedelta(minutes=500), now)
        )
...
"""
    ...
    def test_run_backtest_cli_atr_usage(self):
        ...
        src = inspect.getsource(nicegold.run_backtest_cli)
        self.assertTrue(
            "row['atr'] * sl_multiplier" in src or 'row["atr"] * sl_multiplier' in src
        )
...
class TestModernScalping(unittest.TestCase):
    @unittest.skipUnless(
        pandas_available and numpy_available and sklearn_available,
    ...
    def test_load_config_and_time_filter(self):
        cfg = (
            nicegold.load_config()
            if hasattr(nicegold, "load_config")
            else {
                "trade_start_hour": 2,
                "trade_end_hour": 3,
                "initial_capital": 100.0,
...
class TestNewFunctions(unittest.TestCase):
    ...
    def test_generate_smart_signal_column(self):
        ...
        with patch.object(nicegold, "load_csv_m15") as m15, patch.object(
            nicegold, "detect_ob_m15"
        ) as dob, patch.object(nicegold, "detect_fvg_m15") as dfvg, patch.object(
            nicegold, "detect_liquidity_grab_m15"
        ) as dlg:
            m15.return_value = pd.DataFrame(
                {
            ...
            dfvg.return_value = pd.DataFrame(
                {
                    "type": ["bullish"],
                    "low": [0.9],
                    "high": [1.1],
    ...
    def test_backtest_with_partial_tp_returns(self):
        ...
        with patch.object(
            nicegold,
            "load_csv_m15",
            return_value=df[["timestamp", "close", "high", "low", "open"]],
        ), patch.object(nicegold, "detect_ob_m15", return_value=empty), patch.object(
            nicegold, "detect_fvg_m15", return_value=empty
        ), patch.object(
            nicegold, "detect_liquidity_grab_m15", return_value=empty
        ):
...
class TestOptimizeMemory(unittest.TestCase):
    ...
    def test_optimize_memory_downcast(self):
        df = pd.DataFrame(
            {"a": np.arange(3, dtype="int64"), "b": np.arange(3, dtype="float64")}
        )
...
class TestTrendConfirmFunctions(unittest.TestCase):
    ...
    def test_is_trending_and_confirm(self):
        ...
        self.assertTrue(
            nicegold.is_confirm_bar(df.assign(high=df["high"] * 1.1), 150, "buy")
        )
...
class TestFallbackSimpleSignal(unittest.TestCase):
    ...
    def test_fallback_simple_signal_generates_entry(self):
        df = pd.DataFrame({"macd_hist": [0.1, 0.2, 0.3], "RSI": [60, 60, 60]})
