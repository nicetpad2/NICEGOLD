...
def load_data(file_path=None):
    ...
    df["timestamp"] = pd.to_datetime(ts + " " + df["timestamp"], errors="coerce")
...
def apply_ema_trigger(df, price_col="close"):
    ...
    df["ema_touch"] = np.where(
        (df[price_col] <= df["ema35"] * 1.003) & (df[price_col] >= df["ema35"] * 0.997),
        True,
        False,
    )
    return df
...
def calculate_trend_confirm(df, price_col="close"):
    ...
    df["trend_confirm"] = np.where(
        ema_fast > ema_slow, "up", np.where(ema_fast < ema_slow, "down", "flat")
    )
...
def detect_elliott_wave_phase(
    df, price_col="close", rsi_col="RSI", divergence_col="divergence"
):
    logger.debug("Detecting Elliott wave phase")
    df = df.copy()
    df["Wave_Phase"] = None

    df["zz_high"] = (df[price_col] > df[price_col].shift(1)) & (
        df[price_col] > df[price_col].shift(-1)
    )
    df["zz_low"] = (df[price_col] < df[price_col].shift(1)) & (
        df[price_col] < df[price_col].shift(-1)
    )
...
def generate_entry_signal(df, gain_z_thresh=0.3, rsi_thresh=50):
    ...
    df["entry_signal"] = np.where(
        df["Signal_Score"] >= 2, "buy", np.where(df["Signal_Score"] <= -2, "sell", None)
    )
    ...
    df["entry_signal"] = np.where(
        hybrid_buy, "buy", np.where(hybrid_sell, "sell", df["entry_signal"])
    )
...
def generate_entry_signal_wave_enhanced(df, rsi_buy=52, rsi_sell=48, ema_col="ema35"):
    ...
    buy_cond = (
        ...
        & (df["RSI"] > rsi_buy)
        & (df["close"] >= df[ema_col] * 0.995)
        & (df["close"] <= df[ema_col] * 1.005)
    )
...
def generate_entry_score_signal(df, ema_col="ema35", rsi_threshold=50):
    ...
    ema_prox = (df["close"] >= df[ema_col] * 0.995) & (
        df["close"] <= df[ema_col] * 1.005
    )
...
def apply_wave_macd_cross_entry(df, ema_col="ema35"):
    ...
    for i in range(2, len(df)):
        if pd.isna(df.at[df.index[i], "entry_signal"]):
            if (
                df.at[df.index[i], "Wave_Phase"] in ["W.2", "W.3", "W.5", "W.B"]
                and df.at[df.index[i], "divergence"] == "bullish"
                and df.at[df.index[i], "macd_cross_up"]
                and df.at[df.index[i], "RSI"] > 45
                and df.at[df.index[i], "close"] >= df.at[df.index[i], ema_col] * 0.995
                and df.at[df.index[i], "close"] <= df.at[df.index[i], ema_col] * 1.005
            ):
                df.at[df.index[i], "entry_signal"] = "buy"
            elif (
                df.at[df.index[i], "Wave_Phase"] in ["W.2", "W.3", "W.5", "W.B"]
                and df.at[df.index[i], "divergence"] == "bearish"
                and df.at[df.index[i], "macd_cross_down"]
                and df.at[df.index[i], "RSI"] < 55
                and df.at[df.index[i], "close"] >= df.at[df.index[i], ema_col] * 0.995
                and df.at[df.index[i], "close"] <= df.at[df.index[i], ema_col] * 1.005
            ):
...
def fallback_simple_signal(df, rsi_upper: float = 0, rsi_lower: float = 100):
    ...
    logger.debug(
        "Applying fallback simple signal (upper=%s, lower=%s)", rsi_upper, rsi_lower
    )
    df = df.copy()
...
def compute_features(df):
    ...
    df["rsi"] = (
        ...
        .apply(
            ...
            / (
                1
                + np.mean(np.clip(np.diff(x), 0, None))
...
def train_signal_model(df):
    ...
    df["entry_signal"] = np.where(
        (df["signal_prob"] > 0.55) & (df["atr"] > df["atr"].rolling(50).mean()),
        "buy",
        None,
    )
...
def run_backtest(df, cfg):
    ...
    for i in range(20, len(df)):
        ...
        if not (
            cfg.get("trade_start_hour", 0)
            <= row.get("hour", 0)
            <= cfg.get("trade_end_hour", 23)
        ):
            continue
        ...
        if "volume" in df.columns:
            ...
            vol_mean = 0
        volatility_high = (row["high"] - row["low"]) > 3 * row["atr"] or (
            row.get("volume", 0) > 2 * vol_mean
        )

        if position:
            if not position["tp1_hit"] and (
                (position["type"] == "long" and row["high"] >= position["tp1"])
                or (position["type"] == "short" and row["low"] <= position["tp1"])
            ):
                pnl = (
                    position["lot"]
                    * cfg.get("partial_tp_ratio", 0.5)
                ...
                logger.info(
                    f"Partial TP hit at {position['tp1']:.2f}, lot {position['lot']*cfg.get('partial_tp_ratio',0.5)} closed [Patch]"
                )
            elif (position["type"] == "long" and row["low"] <= position["sl"]) or (
                ...
                logger.info(
                    f"Stop Loss hit at {position['sl']:.2f}, exit trade [Patch]"
                )
                position = None
            elif (position["type"] == "long" and row["high"] >= position["tp2"]) or (
                position["type"] == "short" and row["low"] <= position["tp2"]
            ):
                pnl = (
                    position["lot"]
                    * (position["tp2"] - position["entry"])
                    * (100 if position["type"] == "long" else -100)
                )
                capital += pnl
                trades.append(
                ...
                logger.info(
                    f"Final TP hit at {position['tp2']:.2f}, close remaining position [Patch]"
                )
                position = None
            ...
            if position and position["tp1_hit"]:
                ...
                if (position["type"] == "long" and new_sl > position["sl"]) or (
                    ...
                    position["sl"] = new_sl
                    logger.debug(
                        f"Trailing SL adjusted to {new_sl:.2f} at {row['timestamp']}"
                    )
            ...
            if drawdown * 100 >= cfg.get("max_drawdown_pct", 30):
                logger.warning(
                    "Kill switch triggered - Drawdown {:.2%}".format(drawdown)
                )
                break
        ...
        if position is None:
            ...
            if row["entry_signal"] in ["buy", "sell"]:
                ...
                tp1 = entry_price + (
                    row["atr"]
                    * cfg.get("tp1_mult", 0.8)
                    * (1 if direction == "buy" else -1)
                )
                tp2 = entry_price + (
                    row["atr"]
                    * cfg.get("tp2_mult", 2.0)
                    * (1 if direction == "buy" else -1)
                )
...
def run_backtest_cli():  # pragma: no cover
    ...
    if psutil is not None:
        ...
        logger.info(
            f"[QA] RAM used: {ram_used:.2f} GB"
        )  # [Patch] Show RAM usage for QA monitoring
    ...
    trailing_atr_multiplier = (
        1.0  # ระยะ trailing SL = 1 * ATR [Patch: ATR-based Trailing]
    )
    ...
    kill_switch_threshold = (
        50.0  # [Patch 1] Stop trading if equity falls below $50 (50% drawdown)
    )
    ...
    atr_rolling_mean = (
        df["atr"].rolling(50).mean().bfill().values
    )  # ATR เฉลี่ย 50 แท่ง สำหรับตรวจสอบ volatility [Patch]
    ...
    for i in range(1, len(df)):
        ...
        if not kill_switch_triggered and capital <= kill_switch_threshold:
            ...
            logger.warning(
                f"Kill switch triggered at equity ${capital:.2f}. Stopping trading."
            )
        ...
        if position is None:
            ...
            if (
                row["entry_signal"] in ["buy", "sell"]
                and not row.get("spike", False)
                ...
                if consecutive_loss >= 3:
                    factor_by_streak *= (
                        0.75  # แพ้ต่อเนื่องนาน -> ลดเป้าหมายกำไรลง (เอาให้ออกได้) [Patch]
                    )
                if consecutive_win >= 3:
                    factor_by_streak *= (
                        1.25  # ชนะต่อเนื่องหลายไม้ -> เพิ่มเป้ากำไรให้เหมาะกับเทรนด์ [Patch]
                    )
                ...
                tp = entry_price + (
                    ...
                    * tp_multiplier
                    * factor_by_streak
                    * (1 if position_type == "long" else -1)
                )
                ...
                if position_type == "long":
                    if row.get("LG_Bull", False) and (
                        row.get("OB_Bull", False) or row.get("FVG_Bull", False)
                    ):
                        reason_components.append("MTF-SMC OB+LG+ConfirmBar BUY")
                    elif (
                        str(wave_phase).startswith("W.")
                        and divergence == "bullish"
                        and row.get("macd_cross_up", False)
                    ):
                        reason_components.append(
                            f"WavePhase {wave_phase} + bullish divergence"
                        )
                        reason_components.append("MACD cross up")
                    else:
                        reason_components.append("Bullish signal triggered")
                else:
                    if row.get("LG_Bear", False) and (
                        row.get("OB_Bear", False) or row.get("FVG_Bear", False)
                    ):
                        ...
                        reason_components.append(
                            f"WavePhase {wave_phase} + bearish divergence"
                        )
                ...
                last_entry_idx = i
                logger.info(
                    f"Opened {position_type} position at {position['time']} - Entry: {position['entry']:.2f}, SL: {position['sl']:.2f}, TP: {position['tp']:.2f}, Lot: {position['lot_size']:.2f} | Reason: {reason_entry}"
                )
            elif should_force_entry(
                row,
                (
                    df.iloc[last_entry_idx]["timestamp"]
                    if last_entry_idx >= 0
                    else row["timestamp"]
                ),
                row["timestamp"],
                ...
                if consecutive_loss >= 3:
                    factor_by_streak *= (
                        0.75  # after 3 losses, shorten TP (secure quicker profit)
                    )
                if consecutive_win >= 3:
            ...
            if position["type"] == "long":
                ...
                if not position["tp1_hit"] and row["high"] >= position["entry"] + (
                    position["entry"] - position["sl"]
                ):
                    pnl = (
                        capital * risk_per_trade * 0.5
                    )  # รับกำไรออกครึ่งหนึ่ง (0.5R) [Patch: Partial TP]
                    capital += pnl
                    peak_capital = max(peak_capital, capital)
                    position["sl"] = position[
                        "entry"
                    ]  # เลื่อน SL มาที่ทุน (Breakeven) [Patch: Move SL to BE]
                    position["tp1_hit"] = True
                    logger.info(
                        f"TP1 reached for {position['type']} at {position['time']} - Capital now {capital:.2f}"
                    )
                    trades.append(
                        {
                            **position,
                ...
                if row["high"] >= position["tp"] - pip_size * 0.5:
                    position["sl"] = max(
                        position["sl"], row["close"] - pip_size * 0.5
                    )  # รัด SL เข้ามาใกล้ราคาปัจจุบัน [Patch: Tighten SL near TP]
                # Trailing Stop หลัง TP1 ให้ SL ตามหลังราคาด้วย ATR
                if position["tp1_hit"]:
                    new_sl = position["entry"] + (
                        (row["close"] - position["entry"])
                        - row["atr"] * trailing_atr_multiplier
                    )
                    position["sl"] = max(
                        position["sl"], new_sl
                    )  # เลื่อน SL ตามหลังราคาด้วย ATR [Patch: ATR Trailing Stop]
                # เช็คเงื่อนไขการปิดออเดอร์
                if row["low"] <= position["sl"]:
                    exit_price = position["sl"]
                    direction = 1
                    effective_risk_amount = position["risk_amount"] * (
                        0.5 if position.get("tp1_hit") else 1.0
                    )
                    pnl_move = (
                        direction
                    ...
                    if (
                        ...
                        and abs(position["entry"] - exit_price) > 1e-6
                    ):
                ...
                commission = (position["lot_size"] / lot_unit) * commission_per_lot
                if not position["tp1_hit"] and row["low"] <= position["entry"] - (
                    position["sl"] - position["entry"]
                ):
        ...
        if len(trades) >= 50:
            ...
            if winrate50 < 0.35:
                logger.warning(
                    f"[QA] Recent Winrate {winrate50:.2%} < 35% in last 50 trades -- STRATEGY PAUSED!"
                )
    ...
    if not df_trades.empty:
        ...
        print(
            df_trades[
                ["entry", "exit", "pnl", "capital", "reason_entry", "reason_exit"]
            ].tail(5)
        )
    else:
...
def generate_smart_signal(df):
    ...
    buy_cond = (
        (df["OB_Bull"] | df["FVG_Bull"]) & df["LG_Bull"] & (df["close"] > df["open"])
    )
    sell_cond = (
        (df["OB_Bear"] | df["FVG_Bear"]) & df["LG_Bear"] & (df["close"] < df["open"])
    )
    ...
    for i in range(1, len(df)):
        ...
        if position is None:
            if (
                ...
                and (ts - last_entry_time).total_seconds() > cooldown_minutes * 60
            ):
                atr = row["atr"]
...
def run():
    ...
    df = df.dropna(subset=["atr", "RSI", "ema35"])
    logger.debug(
        "Dropped %d rows with NaN indicators before backtest", before - len(df)
    )
    df = generate_smart_signal(df)
    qa_log_step("Signals generated")
...
def detect_ob_m15(df_m15: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(2, len(df_m15)):
        if (
            df_m15["close"].iloc[i - 1] < df_m15["open"].iloc[i - 1]
            and df_m15["high"].iloc[i]
            > df_m15["high"].iloc[i - 2] + df_m15["atr"].iloc[i] * 0.6
        ):
            obs.append(
                {
                    "type": "bullish",
                    "zone": df_m15["low"].iloc[i - 1],
                    "idx": i - 1,
                    "time": df_m15["timestamp"].iloc[i - 1],
                }
            )
        if (
            df_m15["close"].iloc[i - 1] > df_m15["open"].iloc[i - 1]
            and df_m15["low"].iloc[i]
            < df_m15["low"].iloc[i - 2] - df_m15["atr"].iloc[i] * 0.6
        ):
            obs.append(
                {
                    "type": "bearish",
                    "zone": df_m15["high"].iloc[i - 1],
                    "idx": i - 1,
                    "time": df_m15["timestamp"].iloc[i - 1],
                }
            )
    ...
    for i in range(1, len(df_m15) - 1):
        if df_m15["low"].iloc[i + 1] > df_m15["high"].iloc[i - 1]:
            fvg.append(
                {
                    "type": "bullish",
                    "low": df_m15["high"].iloc[i - 1],
                    "high": df_m15["low"].iloc[i + 1],
                    "idx": i,
                    "time": df_m15["timestamp"].iloc[i],
                }
            )
...
def detect_order_block(df: pd.DataFrame, lookback: int = 50) -> pd.DataFrame:
    ...
    for i in range(2, len(df)):
        if (
            ...
            and df["high"].iloc[i] > df["high"].iloc[i - 2] + df["atr"].iloc[i] * 0.5
        ):
            ob_zones.append(
                {
                    "idx": i - 1,
        ...
        if (
            df["close"].iloc[i - 1] > df["open"].iloc[i - 1]
            and df["low"].iloc[i] < df["low"].iloc[i - 2] - df["atr"].iloc[i] * 0.5
        ):
            ob_zones.append(
                {
                    "idx": i - 1,
...
def is_smc_entry(
    ...
    if (
        not long_lg.empty
        and (not ob_long.empty or not fvg_long.empty)
        and is_confirm_bar(df, i, "buy")
    ):
        return "buy"
    if (
        not short_lg.empty
        and (not ob_short.empty or not fvg_short.empty)
        and is_confirm_bar(df, i, "sell")
    ):
        return "sell"
    return None
